    ...
    <PageWrapper>
      ...
      <PostContainer>
        ...
        <Paragraph>
          ...
          queuing service provided by Amazon Web Services (AWS). It enables the
          decoupling and scaling of microservices, distributed systems, and
          serverless applications. When we start deploying multiple
          applications, they will inevitably need to communicate with one
          another. There are two patterns of communication.
        </Paragraph>
        ...
        <TextList>
          ...
          <TextListItem>
            Asynchronous/Event based communications (application to queue to
            application).
          </TextListItem>
        </TextList>
        ...
        <Paragraph>
          An SQS queue acts as a buffer between producers (anything that sends
          messages to a queue) and consumers (anything that polls messages from
          a queue).
        </Paragraph>
        ...
        <Paragraph>
          Producers sends messages to the queue via the SendMessage API. Once
          the message is sent it is persists in the queue until the consumer
          deletes it or it hits the retention limit.
        </Paragraph>

        <SubSectionHeading>Consumers</SubSectionHeading>
        <Paragraph>
          Consumers are instances that could be EC2 or AWS Lambda or any server.
          This instance polls for SQS messages that can receive up to 10
          messages at a time. Once the message has been processed (whatever
          custom code runs on the server) the consumer will then delete the
        ...
        <Paragraph>
          ...
          consumers. By default, the 'message visibility timeout' is 30 seconds
          which means the message has 30 seconds to be processed. If the message
          is not processed within the visibility timeout, it will be processed
          twice. If the message is in the middle of being processed by a
          consumer but hasn't finished before the timeout, it's possible for the
          consumer to call the ChangeMessageVisibility API to get more time.
          It's possible to change the timeout but keep in mind that if it's too
          high (hours) the consumer can crash and re-processing will take time.
          Alternatively if it's too low (seconds), the chances of duplicate
          messages will be higher.
        </Paragraph>
        ...
        <Paragraph>
          ...
          called Long Polling. Long Polling decreases the number of API calls
          made to SQS while increasing the efficiency and latency of an
          application. The wait time can be anywhere between 1-20 seconds.
        </Paragraph>
        ...
        <TextList>
          ...
          <TextListItem>Enables secure communication over HTTPS.</TextListItem>
          <TextListItem>
        ...
        <Paragraph>
          This is the default option which supports nearly unlimited number of
          API calls per second. The important things to note for this queue are:
        </Paragraph>

        <SubSectionHeading>Attributes</SubSectionHeading>
        <TextList>
          <TextListItem>
            Default retention of messages: A message can stay in a queue for 4
            days by default and a maximum of 14 days.
          </TextListItem>
          <TextListItem>
          ...
          <TextListItem>
            Low latency: Under 10ms on publishing and receiving a message.
          </TextListItem>
        ...
        <Paragraph>
          Consumers will be running on instances inside an auto scaling group
          which will be polling messages from the queue. There is a metric we
          can use to scale up the instances if there is a spike in messages.
          This metric (ApproximateNumberOfMessages) can be tied to a cloudwatch
          alarm, which when is breached can trigger the auto scaling group to
          ramp up capacity.
        </Paragraph>
        ...
        <Paragraph>
          Here is an example of having a separate instance process a message.
          This is a common architecture pattern to prevent any bottleneck to the
          front-end application. In this example the front-end application can
          send a message to a queue and the second tier can poll the message and
          insert it into an S3 bucket.
        </Paragraph>
        ...
        <Paragraph>
          FIFO - First In First Out. This means that it prioritises sending
          messages in the order of which the queue receives those messages.
        </Paragraph>
        ...
        <Paragraph>
          This type of queue is limited to 300 msg/s without batching, 3000
